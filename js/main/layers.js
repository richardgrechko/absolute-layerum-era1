...
function Layer(n) {
  ...
  if (n.gte(1)) {
    ...
    k = `${layers[1][0]}<sup>${Layer(n.sub(10))}</sup>`
  } else if (n.gte(100)) {
    k = `${layers[2][0]}<sub>${Layer(n.sub(100))}</sub>`
  } else if (n.gte(1000)) {
    k = `${Layer(n.div(1000))}<sub>(${Layer(n.mod(1000))})</sub>`
  } else if (n.gte(E(10).pow(9))) {
    let logBillion = n.log(E(10).pow(9));
    k = `${Layer(n.div(E(10).pow(9)))}<sup>{${Layer(n.div(E(10).pow(9))}}</sup>`
...
function update() {
  ...
  tmp.layer = tmp.number.div(tmp.layerRequired.pow(tmp.number.log(tmp.layerRequired).floor()))" " + Layer(tmp.number.log(tmp.layerRequired))
}
