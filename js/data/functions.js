const funcs = {
	...
	update: function()
	{
		...
		game.stats = `<div class="small center" style="color: #900">Epilepsy warning when you get high stats! This is an inspiration of "SamirDevs AFK Incremental"<p></div><div class="small center">Stats: </div><div class="default">${E(5).pow(game.number.log(game.layerRequired).sub(game.number.log(game.layerRequired).floor()))}</div><div class="small center" style="color: ${this.rainbowTransition(number.log(layerRequired).floor().log(1.05), 80, 70)}; text-shadow: 0 0 ${game.number.log(game.layerRequired).floor().div(141)}em currentcolor, 0 0 ${game.number.log(game.layerRequired).floor().div(200)}em currentcolor, 0 0 ${game.number.log(game.layerRequired).floor().div(100)}em currentcolor;">{{Layer(game.number.log(game.layerRequired).floor())}}</div><p><div class="tiny center">(+{{statsPerSecond}} stats/sec)</div>`
