...
const funcs = {
  ...
  update: function() {
    ...
    + Elements.setHTML("", "p") + Elements.setHTML("Stats: " + Elements.setHTML("{{formatNumber(number.log(layerRequired))}}", "default"), "small", {}, "v-if=\"tmp.number.lte(E(5)).pow(52*(53**9))\"") + Elements.setHTML("Stats: ", "small", {}, "v-else") + Elements.setHTML("{{number.log(tmp.layerRequired)}} ", "default", {color: rainbowTransition(tmp.number.log(tmp.layerRequired).floor().log(1.05)), shadowX: 0, shadowY: 0, shadowBlur: (tmp.number.gte(5**100) ? "10px" : (tmp.number.log(tmp.layerRequired).floor().div(10))}) + Elements.setHTML(" (+{{statsPerSecond}} stats/sec)", "small") + Elements.setHTML("", "p") + Elements.setHTML("This is also {{number}} a.", "tiny");
